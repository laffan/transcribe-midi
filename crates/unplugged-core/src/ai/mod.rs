...
// The note diff describes a change to a list of notes and has nothing to do with the
// model; it lives in `crate::diff`. Re-exported so existing `ai::NoteDiff` paths work.
