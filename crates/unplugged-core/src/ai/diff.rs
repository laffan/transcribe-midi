...
//!
//! What a change to a list of notes *is* has nothing to do with the model, so the
//! `NoteDiff` type itself lives in [`crate::diff`]. What is here is the part that does:
//! reading one back out of a finished workspace.

use std::collections::{HashMap, HashSet};

use crate::command::{Command, Transaction};
use crate::diff::{NoteChange, NoteDiff};
