 ...
 * rather than clipped, and the playhead is last so it is never hidden. Splitting the layers
 * into separate files would let that ordering be changed by accident.
 *
 * Pure apart from the context it is handed — no component state, no refs, no reads of the
 * DOM beyond the theme it is given.
 */
...
export interface RollScene {
  ...
  width: number;
  height: number;
  /** Height of the note area — the velocity lane sits below it. */
  rollHeight: number;
  view: Viewport;
  track: Track;
...
export function paintRoll(ctx: CanvasRenderingContext2D, scene: RollScene): void {
  ...
  const { rowHeight, scrollTicks, topPitch, pxPerTick } = view;
  ...
  track.notes.forEach((note, index) => {
    ...
    const width_ = rect.width - (x - rect.x);
    ctx.fillRect(x, rect.y + 1, Math.max(1, width_), rect.height - 2);

    ctx.globalAlpha = 1;
    if (selected && !preview) {
      ctx.strokeStyle = theme.keyWhite;
      ctx.lineWidth = 1;
      ctx.strokeRect(x + 0.5, rect.y + 1.5, Math.max(1, width_) - 1, rect.height - 3);
    }
  });
  ctx.globalAlpha = 1;
  ...
  if (preview) {
    const block = (note: Note, color: string, filled: boolean) => {
      ...
      const x = Math.max(KEY_WIDTH, rect.x);
      const blockWidth = Math.max(1, rect.width - (x - rect.x));

      if (filled) {
        ctx.globalAlpha = 0.85;
        ctx.fillStyle = color;
        ctx.fillRect(x, rect.y + 1, blockWidth, rect.height - 2);
        ctx.globalAlpha = 1;
      } else {
        ctx.globalAlpha = 0.9;
        ...
        ctx.lineWidth = 1;
        ctx.setLineDash([3, 2]);
        ctx.strokeRect(x + 0.5, rect.y + 1.5, blockWidth - 1, rect.height - 3);
        ctx.setLineDash([]);
        ctx.globalAlpha = 1;
      }
    };

    preview.removed.forEach((note) => block(note, theme.diffRemoved, false));
    preview.changed.forEach(({ before }) => block(before, theme.diffChanged, false));
    preview.changed.forEach(({ after }) => block(after, theme.diffChanged, true));
    preview.added.forEach((note) => block(note, theme.diffAdded, true));
  }

  // ---- velocity lane ----
  const lane = velocityLane(rollHeight);
  ctx.fillStyle = theme.bg1;
  ctx.fillRect(0, lane.top, width, VELOCITY_LANE_HEIGHT);
  ctx.strokeStyle = theme.border;
  ctx.beginPath();
  ctx.moveTo(0, lane.top + 0.5);
  ctx.lineTo(width, lane.top + 0.5);
  ctx.stroke();

  ctx.fillStyle = theme.text2;
  ctx.font = "10px ui-monospace, monospace";
  ctx.fillText("VELOCITY", 6, lane.top + 14);
  ...
  track.notes.forEach((note, index) => {
    ...
    ctx.fillRect(x, lane.bottom - barHeight, 3, barHeight);
  });
  ctx.globalAlpha = 1;
