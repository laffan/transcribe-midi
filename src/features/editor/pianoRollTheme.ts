 ...
 * document and handed over as strings. Reading them per frame rather than caching is what
 * makes the roll follow a theme change without a remount; each fallback matches the value
 * in `src/styles/tokens.css`, so a missing token degrades to the intended colour rather
 * than to transparent black.
 */
